...
public class EmailConfig {
    ...
    public boolean MailDenyTrial(String recipientEmail, String name) {
        String emailUsername = "ducanhqbz@gmail.com"; // email cua ban than
        String emailPassword = "pfeb gdjy vrnu tiva"; // mat khau ung dung
        ...
        try {
            ...
            email.setFrom(emailUsername, "Tutorly.com"); // Nguoi gui
            email.setSubject("Confirmation"); // Tieu de
            email.setMsg("Your Request to Learn with" + name + "was denied"); // Noi dung
            email.addTo(recipientEmail); // Dia chi email can gui toi
            email.send(); // gui
            return true; // tra ve true
        } catch (EmailException e1) {
            ...
            try {
                ...
                email.setMsg("Your Request to Learn with" + name);
                email.addTo(recipientEmail);
                email.send();
                return true;
            } catch (EmailException e2) {
      ...
      public boolean MailAgreeTrial(String recipientEmail, String name) {
        String emailUsername = "ducanhqbz@gmail.com"; // email cua ban than
        String emailPassword = "pfeb gdjy vrnu tiva"; // mat khau ung dung
        ...
        try {
            ...
            email.setFrom(emailUsername, "Tutorly.com"); // Nguoi gui
            email.setSubject("Confirmation"); // Tieu de
            email.setMsg("Your Request to Learn with" + name + "was Accepted"); // Noi dung
            email.addTo(recipientEmail); // Dia chi email can gui toi
            email.send(); // gui
            return true; // tra ve true
        } catch (EmailException e1) {
